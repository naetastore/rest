...
function get_product_act_allowed($array)
{
	$ci = get_instance();

	$status_id = $ci->db->get_where('product_status', [ 'status' => $array['status'] ])->row_array()['id'];
	$action_id = $ci->db->get_where('product_action', [ 'action' => $array['action'] ])->row_array()['id'];

	$dataQuery = [
		'status_id' => $status_id,
		'action_id' => $action_id
	];
	$result = $ci->db->get_where('product_action_allowed', $dataQuery);
	return $result;
}

function check_product_action_allowed($status_id, $action_id)
{
	$ci = get_instance();
	...
	$result = $ci->db->get_where('product_action_allowed', $dataConfig);
	if ($result->num_rows() > 0) {
		return "checked=checked";
	}
}

function get_api_setting($setting)
{
	$ci = get_instance();
	return $ci->db->get_where('api_settings', ['setting' => $setting])->row_array()['value'];
}
